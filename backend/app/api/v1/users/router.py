...
from .schemas import CreateUser, GetUser, UpdateUserPartial, UserData
...
async def get_user_by_id(user_id: uuid.UUID, user_dao: UserDAO = Depends()) -> UserData:
...
async def get_user_by_email(user_data: GetUser, user_dao: UserDAO = Depends()) -> UserData:
...
async def create_user(user_data: CreateUser, user_dao: UserDAO = Depends()) -> UserData:
...
async def update_user_partial(user_id: uuid.UUID, update_data: UpdateUserPartial, user_dao: UserDAO = Depends()) -> UserData:
    user = await user_dao.get_by_id(user_id=user_id)
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
...
async def delete_user(user_id: uuid.UUID, user_dao: UserDAO = Depends()):
    user = await user_dao.get_by_id(user_id=user_id)
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
