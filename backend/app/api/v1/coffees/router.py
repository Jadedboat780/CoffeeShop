...
from .schemas import CreateCoffee, GetCoffee, UpdateCoffee, UpdateCoffeePartial

router = APIRouter(prefix="/coffees", tags=["Coffees"])


@router.get("/{coffee_id}", status_code=status.HTTP_200_OK)
async def get_coffee(coffee_id: Annotated[int, Path(ge=1)], coffee_dao: CoffeeDAO = Depends()):
...
async def get_coffees(pagination: Paginator = Depends(), coffee_dao: CoffeeDAO = Depends()) -> list[GetCoffee]:
...
async def create_coffee(coffee_data: CreateCoffee, coffee_dao: CoffeeDAO = Depends()):
...
async def delete_coffee(coffee_id: Annotated[int, Path(ge=1)], coffee_dao: CoffeeDAO = Depends()):
    coffee = await coffee_dao.get_by_id(coffee_id=coffee_id)
    if coffee is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
